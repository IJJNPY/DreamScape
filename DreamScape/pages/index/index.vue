		...
		<view class="banner">
			...
			indicator-active-color="#fff" autoplay>
				<swiper-item v-for="item in bannerList" :key="item._id">
					...
					<view v-else class="like" @click="routerTo(item.url)">
						<image :src="item.picurl" mode="aspectFill"></image>
					</view>
				</swiper-item>				
			</swiper>
...
const bannerCloudObj = uniCloud.importObject("client-wallpaper-banner",{customUI:true});
...
const getBanner = async ()=>{	
	let {errCode,data} = await bannerCloudObj.list();
	if(uni.getSystemInfoSync().uniPlatform != 'mp-weixin'){
		data = data.filter(item=>item.target=='')
	}
	if(errCode!=0) return showToast({title:"获取banner失败"})
